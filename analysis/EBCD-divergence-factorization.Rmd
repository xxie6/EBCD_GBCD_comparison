...
```{r}
plot_heatmap(t(data_norm_patient_sd0.01$FF) %*% data_norm_patient_sd0.01$FF)
```

```{r}
observed.vals_patient_sd0.01 <- data_norm_patient_sd0.01$Y %*% t(data_norm_patient_sd0.01$Y)/ ncol(data_norm_patient_sd0.01$Y)
```
...
```{r}
ebcd.laplace.fitted.vals_patient_sd0.01 <- fit.ebcd.laplace_patient_sd0.01$EL %*% t(fit.ebcd.laplace_patient_sd0.01$EL)
```
...
```{r}
plot_heatmap(t(data_norm_patient_sd0.1$FF) %*% data_norm_patient_sd0.1$FF)
```
...
```{r}
ebcd.laplace.fitted.vals_patient_sd0.1 <- fit.ebcd.laplace_patient_sd0.1$EL %*% t(fit.ebcd.laplace_patient_sd0.1$EL)
```
